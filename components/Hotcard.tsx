...
export const Card = ({ card, onPlay, onDownload }: CardProps) => {
  return (
    <div className="w-full h-full min-h-96 relative flex flex-col shrink-0 overflow-hidden rounded-2xl shadow-xl bg-slate-900 group border border-slate-800">
      ...
      <div className="absolute bottom-0 left-0 right-0 top-[60%] z-10 p-5 sm:p-6 flex flex-col justify-between bg-slate-950/90 backdrop-blur-md border-t border-slate-800/80">
        <div>
          <h3 className="text-white font-bold text-base sm:text-lg tracking-tight leading-snug line-clamp-2">
            {card.title}
          </h3>
        </div>

        <div className="flex justify-between items-center mt-3">
          <span className="text-slate-400 text-xs sm:text-sm font-medium">{card.date}</span>
          <button
            onClick={(e) => {
              e.stopPropagation();
