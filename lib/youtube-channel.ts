...
export const DEFAULT_CHANNEL_ID = 'UCDwZ_ENzU7LIDA5F8EYf1Jg';
...
async function fetchAllVideosWithInnertube(channelId: string): Promise<YouTubeVideo[]> {
  try {
    ...
    return videos.slice(0, 200).map((v) => ({ ...v, type: 'official' as const, durationSeconds: 0 }));
  } catch (err) {
...
let cachedFeed: { feed: ChannelVideoFeed; fetchedAt: number } | undefined;
const CHANNEL_FEED_TTL_MS = 10 * 60 * 1000;

export async function fetchChannelVideos(
  channelId: string = DEFAULT_CHANNEL_ID,
  options: { force?: boolean } = {},
): Promise<ChannelVideoFeed> {
  if (!options.force && cachedFeed && Date.now() - cachedFeed.fetchedAt < CHANNEL_FEED_TTL_MS) {
  ...
  if (getYoutubeApiKey()) {
    try {
      ...
      const feed: ChannelVideoFeed = { videos, shorts: [], fallback: false };
      cachedFeed = { feed, fetchedAt: Date.now() };
      return feed;
    } catch (err) {
      errorMessage = err instanceof Error ? err.message : 'Unknown error occurred';
      console.warn(`YouTube videos API failed, using fallback: ${errorMessage}`);
    }
  }
  ...
  const feed: ChannelVideoFeed = {
    videos: fallbackFeed.videos,
    shorts: fallbackFeed.shorts,
    fallback: !liveFeed,
  };
  if (errorMessage) feed.error = errorMessage;
